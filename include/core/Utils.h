...
inline bool getLocalLanguageIsCN()
{
    ...
    return std::any_of(std::begin(strs), std::end(strs), [](const std::string &str) { return isContain(str, "zh") || isContain(str, "CN"); });
}
...
inline std::string getRandomString(int length)
{
    ...
    for (int i = 0; i < length - 1; i++)
    {
        switch (rand() % 3)
        {
        case 0: str += 'A' + rand() % 26; break;
        case 1: str += 'a' + rand() % 26; break;
        case 2: str += '0' + rand() % 10; break;
        default: str += 'x'; break;
        }
    }
    return str;
...
inline std::string fileNameFromPath(const std::string &filePath)
{
    ...
    if (std::string::npos != last_slash_idx) filename = filePath.substr(last_slash_idx + 1);
    return filename;
}
...
inline std::string fileDirectory(const std::string &filePath)
{
    ...
    const size_t last_slash_idx = filePath.find_last_of("/\\");
    if (std::string::npos != last_slash_idx) directory = filePath.substr(0, last_slash_idx);
    return directory;
}
...
inline std::string fileExtension(const std::string &filePath)
{
    std::string ext;
    const size_t last_slash_idx = filePath.find_last_of(".");
    if (std::string::npos != last_slash_idx) ext = filePath.substr(last_slash_idx + 1);
    return ext;
}
...
inline bool ValidateHex(std::string &hex)
{
    ...
    if (hex.compare(0, 2, "0x") == 0) hex.erase(0, 2);
